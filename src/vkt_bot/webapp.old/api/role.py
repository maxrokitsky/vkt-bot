...
from vkt_bot.webapp.schemas.role import CreateRoleAPISchema, DetailRoleAPISchema
...
async def create_role(
    session: DB, _user: CurrentUser, role_data: CreateRoleAPISchema
) -> Any:
    """Создать роль."""
    role_repository = RoleRepository(session)
    ...
    return await role_repository.create(
        CreateRoleSchema(name=role_data.name), commit=True
    )
