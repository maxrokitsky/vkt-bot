...
const { data: logs, isPending: logsPending } = useQuery(
  computed(() => ({
    ...listEventsApiEventsGetOptions({ query: { page: 1, size: 8 } }),
    enabled: isAdmin.value,
  })),
)

/** Подписи типов берём из реестра — своего словаря во фронтенде нет. */
...
const totalActions = computed(() => activity.value.reduce((sum, point) => sum + point.count, 0))
...
<template>
  <div class="space-y-8">
    ...
    <PageSection v-if="isAdmin" title="Последние события">
      <template #actions>
    ...
    <PageSection v-else title="Что дальше">
      <p class="text-sm text-muted-foreground">
        Роли и состав чатов — в разделе «Чаты и люди». Вебхуки для внешних систем — в «Интеграциях».
      </p>
    </PageSection>
  </div>
</template>
