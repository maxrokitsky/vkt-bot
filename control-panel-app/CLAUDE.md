...
- `components/data/` — общие примитивы списков: `DataToolbar`,
  ...
  `RowActions`, `StatTile`, `UserAvatar` (картинка из профиля, иначе
  инициалы: ссылка на аватар приходит всегда, а картинка за ней — нет).
...
- `lib/` — `format` (даты), `plural` (склонения), `users` (инициалы для
  аватара — их показывает `UserAvatar`, когда картинки нет), `events`,
  `chats` и `ai` (подписи перечислений API по-русски).
