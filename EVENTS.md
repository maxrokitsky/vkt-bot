...
- **Настройки `events_store_message_text` нет.** Она должна была
  включать хранение текстов, но хранить их оказалось негде: поток
  сообщений в базу не пишется вовсе (`persist=False`), а остальные
  события текстов не несут. Настройка, которая ничего не включает, хуже
  её отсутствия. Отсев текстов на выдаче (`TEXT_FIELDS`) при этом
  остался — payload плагинов и вебхуков может их принести.
