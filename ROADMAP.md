...
### 2.0 Research — остался один вопрос
...
Отсюда дизайн 2.2: отличить событие из треда от события чата по самому
событию нельзя — только по тому, что для `chatId` работает
...
пишут через `chat_id=thread_id`.

### 2.2 Диспетчер — отложено

`ThreadFilter` / `Filter.thread` делать не на чем: пока неизвестно, чем
событие из треда отличается от события чата (см. 2.0). Хелпер «создать тред
к сообщению и ответить в него» — вместе с 2.4.
...
- Модель `Thread` (`thread_id`, `parent_chat_id`, `root_msg_id`,
  `created_at`, связь с сущностью-источником) + миграция — нужна для 2.4,
  чтобы связывать тред с тем, из-за чего он создан. Заполнять её можно
  только для тредов, созданных ботом: родителя чужого треда API не отдаёт.
...
- ✅ Тесты: клиент (`tests/client/test_threads.py`), автоподписка при
