...
- **Бюджет токенов спрашивается дважды** — в хендлере, чтобы не ставить
  заведомо отказную задачу, и в воркере перед вызовом модели. Списание
  идёт после ответа модели, поэтому между постановкой и запуском очередь
  успевает выбрать лимит соседними сессиями: без второй проверки на
  разборе накопившейся очереди он перешагивался бы на столько задач,
  сколько успели поставить. Гонка этим не закрыта полностью — для этого
  нужен атомарный резерв квоты, — но окно сузилось до одной сессии.
