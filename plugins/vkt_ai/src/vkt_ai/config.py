...
class AiSettings(BaseSettings):
    ...
    #: Отвечать на упоминание `@бот` без команды. Команда — барьер: в
    #: живом чате боту пишут как человеку.
    reply_on_mention: bool = True
    #: Отвечать, когда отвечают на сообщение бота. В чате с потоком
    #: уведомлений это стоит денег: люди обсуждают уведомление между
    #: собой, отвечая на него, и каждая такая реплика — вызов модели.
    reply_on_reply: bool = True
    #: Отвечать в личке на любое сообщение, не только на команду. По
    #: умолчанию выключено: в личке под это попадает каждая реплика, а
    #: каждая реплика — это оплаченный вызов модели.
    reply_in_private: bool = False
