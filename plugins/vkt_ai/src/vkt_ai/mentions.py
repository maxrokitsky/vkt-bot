...
**Почему не по `parts`.** В событии упоминание приходит отдельным
элементом `parts` — там есть готовый `userId`. Но `NewMessagePayload` не
разбирает `parts` вовсе: тот же дефект, из-за которого лежат 11 фильтров
в `vkt_dispatcher.filters` (ROADMAP 3.1). Разбор `parts` — отдельная
работа; чинить её ради упоминания необязательно.

**Что используем вместо.** Два независимых способа, чтобы не зависеть от
того, как именно сервер отрисует упоминание в тексте:

1. **Разметка `format.mention`** — она-то моделью разбирается. Оттуда
   известно, какой кусок текста является упоминанием; остаётся посмотреть,
   не про нас ли он. Способ переживает любой вид отрисовки: и `@[id]`, и
   имя человека.
2. **Текст напрямую** — `@[user_id]` или `@nick`. Нужен, когда разметки
   нет: ник печатают руками, не выбирая из списка.
"""
